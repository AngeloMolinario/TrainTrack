...
async def create_loss_batch(loss_batch: schemas.LossBatchCreate, db: AsyncSession = Depends(get_db)):
    losses = [models.Loss(**loss.dict()) for loss in loss_batch.losses]
    db.add_all(losses)
    await db.commit()
    for loss in losses:
        await db.refresh(loss)
...
@router.get("/{run_id}", response_model=List[schemas.LossRead])
async def get_losses(run_id: str, split: str = None, limit: int = None, db: AsyncSession = Depends(get_db)):
    stmt = select(models.Loss).where(models.Loss.run_id == run_id)
    if split:
        stmt = stmt.where(models.Loss.split == split)
    if limit:
        stmt = stmt.limit(limit)
    stmt = stmt.order_by(models.Loss.step)
